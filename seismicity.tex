...
\begin{figure*}[t]
	...
	\caption{\myrevision{Instrumental} seismicity of northern Iran for the 2005--\myrevision{2016} period, considering only the events in the seismic zones of interest, namely Azerbaila, Alborz, and Kopeh Dagh. The size of symbols are proportional to the magnitude of the events, as indicated by the artificial scale shown on the right. The background shows fault lines and the shaded relief. The color version of this figure is available only in the electronic edition.}
	\label{fig:seismicity}
\end{figure*}



It follows from this description that the region of interest is one of significant seismic activity, which goes back centuries. We are, however, interested in the most recent instrumental seismicity. In particular, we focus on events recorded in the last decade, between January 2005 and December \myrevision{2016}. We focus our analysis on this magnitude-time sequence period because in a previous study we observed that the latest decade of seismic records in northern Iran offered the most complete dataset of recorded events, especially for the case of small $M < 4$ earthquakes \myrevision{\citep[][]{Khoshnevis2017}}. 

We compiled a catalog of all recorded earthquakes using data downloaded from the International Institute of Earthquake Engineering and Seismology, IIEES (see the Data and Resources section). \myrevision{Instrumental catalog provided by IIEES} contained a mixture of earthquake magnitude scales, including: moment, $M_w$; local, $M_L$; body wave, $m_b$; surface wave, $M_s$; and duration $M_D$ magnitudes. \myrevision{All earthquakes of this study are reported in local $M_L$ magnitude scale. However, in order to keep consistency with other studies (e.g., completeness magnitude) we converted them to moment magnitude, $M_w$}. Magnitudes were converted using the relationships defined in \citet{Zare2014}.
...
Figure \ref{fig:completeness} shows the \cmmnt{goodness-of-fit} \myrevision{$M_c\ vs\ Time$} values for the three seismic zones. We indicate the selected value of $M_c$ for each region in the figure. We also obtained the $b$-value in the Gutenberg-Richter law for each region following the maximum likelihood estimation \citep{Aki1965}:
\myrevision{
% 
\begin{equation}
	b = \frac{log_{10}(e)}{\overline{M} - (M_{min} - \frac{\Delta M_{bin}}{2})} \, ,
\end{equation}
}
% 
where $e$ is the mathematical constant or Euler's number, $\overline{M}$ is the average magnitude, and $M_{\min}$ is the minimum magnitude in the sample. Here, the value of $M_{\min}$ is taken as the minimum completeness magnitude $M_c$ obtained or selected for each region's sub-catalog. \myrevision{In this study we assumed $\Delta M_{bin} = 0.1$}. Table \ref{tab:seismicity} shows the corresponding values of $M_c$ and $b$.\myrevision{The standard deviation of the estimate of b is calculated by using \citet{Shi1982standard} formula:}
\myrevision{
\begin{equation}
	\sigma b = 2.3b^2\sqrt{\frac{\sum_{i=1}^{N}(M_i - \overline{M})^2}{N(N-1)}},
\end{equation}
where $N$ is the number of events in each catalog.
}
...
\begin{figure}%[t]
	\centering
	\includegraphics[width=0.8\textwidth]{figures/pdf/figure-04-rev.pdf} 
	
	\caption{\myrevision{Completeness magnitude vs time results and selected $M_c$ values for the three tectonic seismic regions in northern Iran as evaluated from the time-magnitude sequence of earthquakes during the 2005--2016 period.}}
	\label{fig:completeness}
\end{figure} 
...
\begin{table}
	...
	\caption{Total number of earthquakes \myrevision{(before and after considering completeness magnitude)} and seismic parameters $M_c$ and $b$ for the three seismic zones of northern Iran.}
	\centering\small
	\myrevision{
	\begin{tabular}{|ccccc|}
		\hline
		Region           & Total Num.  & $M_c$  & Num. of Events          & $b-value$ \\ 
		                      & of Events     &             & in Complete Catalog  &                  \\
		\hline
		Azerbaijan     &        1047     &  3.5       &        202                     &    0.85491 $\pm$ 0.0506 \\
		Alborz            &        1898     &  3.0       &        575                     &    0.81578 $\pm$ 0.0279 \\
		Kopeh Dagh  &         640      &  3.5       &        117                     &    0.78297 $\pm$ 0.0494 \\
		\hline
	\end{tabular}
	}
...
In addition to using $M_c$ to determine the $b$-value for each sub-catalog, we also use $M_c$ as the threshold for limiting the minimum magnitude to be considered in the construction of the visibility graph of each region. The resulting number of events for each region is included in Table \ref{tab:seismicity}. Figure \ref{fig:mag-time} shows the final time-magnitude sequences for Azerbaijan, Alborz and Kopeh Dagh. Note that in the case of \myrevision{Azerbaijan} and Kopeh Dagh, we consider events in the \cmmnt{declustered} catalog down to $M_w \geq 3.5$, whereas in the case of \myrevision{Alborz} all events are $M_w \geq 3.0$.
